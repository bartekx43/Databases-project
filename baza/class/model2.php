...
class model2 
{
   ...
   {
      self::$db = new PDO ( self::$dsn, 'u9bardian', '') ;//interfejs języka PHP przeznaczony do komunikacji z bazami danych
   ...
   public function findBadanie($obj)
   {   
   ...
   {
      $this->sth = self::$db->prepare('UPDATE project.badanie SET data_zakonczenia=NOW() WHERE badanie_id='.$id);
      $resp = ( $this->sth->execute() ? 'true' : 'false' ) ;
      return $resp ; 
   }
}
